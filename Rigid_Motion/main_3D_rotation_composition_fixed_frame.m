...
% Composition of 3D Rotation with respect to Fixed Frame (x-y-z axis sequence)
deg2rad = pi/180;
...
for i = 1:1:300
    ...
    if t < 1.0
        ...
        R = Rz*Ry*Rx;   % Composition of 3D Rotation with respect to Fixed Frame (x- axis rotation)
    ...
    elseif (t > 1.0) && (t <= 2.0)
        ...
        Ry = [cos(beta) 0 sin(beta); 0 1 0; -sin(beta) 0 cos(beta)];
        Rz = [cos(gamma) -sin(gamma) 0; sin(gamma) cos(gamma) 0; 0 0 1];
        R = Rz*Ry*Rx;   % Composition of 3D Rotation with respect to Fixed Frame (y- axis rotation)
    ...
    elseif (t > 2.0) && (t <= 3.0)
        ...
        Ry = [cos(beta) 0 sin(beta); 0 1 0; -sin(beta) 0 cos(beta)];
        Rz = [cos(gamma) -sin(gamma) 0; sin(gamma) cos(gamma) 0; 0 0 1];
        R = Rz*Ry*Rx;   % Composition of 3D Rotation with respect to Fixed Frame (z- axis rotation)
    end
    ...
    ylabel('y- axis')
    zlabel('z- axis')
    title('Composition of Rotation about Fixed Frame (x-y-z)')
    set(gca,'fontsize',13)
    axis equal
    axis([-1 1 -1 1 -1 1])
    grid on
