...
% PD controller with motor angle feedback in Laplace and Time domain
...
% System Transfer function with motor angle feedback --> Input: desired step angle, Output: load angle
den_cl = [Jl*Jm Jl*Bm+Jm*Bl+Jl*Kd (k*(Jl+Jm)+Bl*Bm+Kd*Bl+Kp*Jl) k*(Bl+Bm)+k*Kd+Kp*Bl Kp*k];
num_cl = [k*Kp];
...
for i = 1:1:length(time)
    t = time(i);
    u = Kp*(th_d - th_m) - Kd*dth_m;                        % PD controller with motor angle feedback
...
figure(1)
h3 = plot(time, th_l_out,'r:','linewidth',3);
h4 = plot(time, th_m_out,'b:','linewidth',3);
