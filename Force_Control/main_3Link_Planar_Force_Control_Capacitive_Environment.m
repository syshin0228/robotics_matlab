...
% Force control with 3 Link Planar Manipulator in Capacitive environment (constraint in y axis)
...
P = [0 0 0]';           % Initialize current end-effector position
...
P_e = [0.8 0.9]';       % Position of environment
...
for i = 1:1:length(time)
    ...
    % Environment Model
    if P(2) > P_e(2)
        F_ex(1,:) = 0.0;                    % Free in x- axis
        F_ex(2,:) = Ke*(P_e(2) - P(2));     % Capacitive in y- axis
    ...
    t_p = 1.0;
    if t < t_p  % Position control untime t = t_p sec
        a_x(1,:) = -Kd*dP(1) + Kp*(P_d(1) - P(1));
        a_x(2,:) = -Kd*dP(2) + Kp*(P_d(2) - P(2));
        a_x(3,:) = -Kd*dP(3) + Kp*(alpha_d - alpha);

    else % Start force control
        
        a_x(1,:) = -Bf*dP(1) + Fd(1);
        a_x(2,:) = -Bf*dP(2) + Fd(2);                 
        a_x(3,:) = -Bf*dP(3) + Fd(3);
        
%         % To maintain desired x- position and orientation
%         a_x(1,:) = -Bf*dP(1) + Kf*(P_d(1) - P(1));
%         a_x(2,:) = -Bf*dP(2) + Fd(2);                 
%         a_x(3,:) = -Bf*dP(3) + Kf*(alpha_d - alpha);
...
h102 = plot(time, ones(length(time),1)*-Fd(2),'r:','linewidth',1);
ylabel('External Force [N]')
xlabel('Time [Sec]')
legend([h100, h101, h102],'Fx', 'Fy', 'Fy Desired')
set(gca,'fontsize',13)
...
for i = 1:step:length(time)
    ...
    % Draw Desired Posture
    L_od = 0.1;
    Ori_d = P_d + [L_od*cos(alpha_d) L_od*sin(alpha_d)]';
    plot3(P_d(1), P_d(2), 0, 'bo','linewidth',1)
    line([P_d(1) Ori_d(1)],[P_d(2) Ori_d(2)],[0 0], 'color', 'b', 'linewidth', 1)

    x_lim = [-0.6 1.6];
    y_lim = [-0.6 1.6];
    
    % Draw Environment 
    line(x_lim,[P_e(2) P_e(2)],'color','k','linewidth', 1,'linestyle', ':')
    line(x_lim,[P_ae_out(i,2) P_ae_out(i,2)],'color','k','linewidth', 3,'linestyle', '-')
    
    % Draw applied force vector
    scale = 1/max(2*abs(F_ex_out(:,2)));
    F_vec = -scale*F_ex_out(i,2);
    line([P_ae_out(i,1) P_ae_out(i,1)],[P_ae_out(i,2) P_ae_out(i,2)+F_vec],'linewidth',2,'color','r')
